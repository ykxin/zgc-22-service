<template>
  <div>
    <NavBar active-menu="/employer/orders" />
    <div class="page-container">
      ...
      <el-table :data="orders" stripe style="width:100%" v-loading="loading">
        ...
        <el-table-column label="操作" min-width="200">
          <template #default="{ row }">
            <el-button v-if="row.status === 'done'" type="success" size="small" @click="openReview(row)">
              评价
            </el-button>
            <el-button v-if="row.status === 'pending'" type="danger" size="small" @click="cancelOrder(row)">
              取消
            </el-button>
            <el-button size="small" @click="openDispute(row)" type="warning">
              纠纷
            </el-button>
    ...
    <el-dialog v-model="detailVisible" title="订单详情" width="600px">
      <div v-if="detail">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="验收得分">{{ detail.acceptance_score }}</el-descriptions-item>
    ...
    <!-- 评价弹窗 -->
    <el-dialog v-model="reviewVisible" title="服务评价" width="400px">
      <el-form>
        ...
        <el-form-item label="标签">
          <el-input v-model="reviewForm.tags" placeholder="逗号分隔，如：专业,细心,准时" />
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <el-dialog v-model="disputeVisible" title="发起纠纷" width="400px">
      ...
      <template #footer>
        <el-button type="warning" :loading="disputing" @click="submitDispute">提交纠纷</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
const statusLabel = (s) => ({
  ...
  done: '待评价', completed: '已完成', cancelled: '已取消'
}[s] || s)
...
async function cancelOrder(row) {
  try {
    await updateOrderStatus(row.id, 'cancelled', userStore.token)
...
async function submitDispute() {
  ...
  try {
    const res = await createDispute(userStore.token, disputeForm)
    ElMessage.success('纠纷已提交，AI已完成判定')
